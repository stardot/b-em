...
//        bem_log(LOG_DEBUG, "Write IDE %04X %02X %04X\n",addr,val,pc);
        switch (addr & 0xF)
        {
                case 0x0:
                ...
                ide.command = val;
                ide.error = 0;
//                bem_log(LOG_DEBUG, "IDE command %02X\n",val);
                switch (val)
                {
                        case 0x10: /*Restore*/
                        case 0x70: /*Seek*/
                ...
                bem_log(LOG_DEBUG, "Bad IDE command %02X", val);
                exit(-1);
                return;
        }
}
...
//        bem_log(LOG_DEBUG, "Read IDE %04X %04X %02X\n",addr,pc,ide.atastat);
        switch (addr&0xF)
        {
                case 0x0:
