...
void adf_readaddress(int drive, int track, int side, int density)
{
        ...
        adf_track = track;
        adf_side  = side;
//        bem_log(LOG_DEBUG, "Read address %i %i %i\n",drive,side,track);
...
//                        exit(-1);
        ...
        if (adf_inreadaddr && adf_f[adf_drive])
        {
                switch (adf_readpos)
                {
                        ...
                        adf_inreadaddr = 0;
                        fdc_finishread();
//                        bem_log(LOG_DEBUG, "Read addr - %i %i %i %i 1 0 0\n",adfdrive,adftrack,adfside,adfsector);
                        adf_rsector++;
