...
//        bem_debugf("Seek %i %i %i %i %i %i\n",drive,track,adfsectors[drive],adfsize[drive],adl[drive],adfsectors[drive]*adfsize[drive]);
	if (adf_dblstep[drive])
		track /= 2;
	adf_trackc[drive] = track;
	if (adl[drive]) {
		fseek(adf_f[drive],
		    track * adf_sectors[drive] * adf_size[drive] * 2,
		    SEEK_SET);
		fread(trackinfoa[drive][0],
		    adf_sectors[drive] * adf_size[drive], 1, adf_f[drive]);
		fread(trackinfoa[drive][1],
		    adf_sectors[drive] * adf_size[drive], 1, adf_f[drive]);
	} else {
		fseek(adf_f[drive],
		    track * adf_sectors[drive] * adf_size[drive], SEEK_SET);
		fread(trackinfoa[drive][0],
		    adf_sectors[drive] * adf_size[drive], 1, adf_f[drive]);
	}
}
...
void adf_writeback(int drive, int track)
{
	...
	if (adl[drive]) {
		fseek(adf_f[drive],
		    track * adf_sectors[drive] * adf_size[drive] * 2,
		    SEEK_SET);
		fwrite(trackinfoa[drive][0],
		    adf_sectors[drive] * adf_size[drive], 1, adf_f[drive]);
		fwrite(trackinfoa[drive][1],
		    adf_sectors[drive] * adf_size[drive], 1, adf_f[drive]);
	} else {
		fseek(adf_f[drive],
		    track * adf_sectors[drive] * adf_size[drive], SEEK_SET);
...
void adf_readsector(int drive, int sector, int track, int side, int density)
{
	...
	adf_side = side;
	adf_drive = drive;
	if (adf_size[drive] != 256)
		adf_sector--;
//        printf("ADFS Read sector %i %i %i %i   %i\n",drive,side,track,sector,adftrackc[drive]);

	if (!adf_f[drive] || (side && !adl[drive]) || !density
	    || (track != adf_trackc[drive])) {
...
//                if (!adfreadpos) bem_debugf("%i\n",adfsector*adfsize[adfdrive]);
		fdc_data(trackinfoa[adf_drive][adf_side][(adf_sector *
			    adf_size[adf_drive]) + adf_readpos]);
		adf_readpos++;
...
//                        exit(-1);
		...
		trackinfoa[adf_drive][adf_side][(adf_sector *
			adf_size[adf_drive]) + adf_readpos] = c;
		adf_readpos++;
		if (adf_readpos == adf_size[adf_drive]) {
			adf_inwrite = 0;
			fdc_finishread();
	...
	if (adf_inreadaddr && adf_f[adf_drive]) {
		...
		case 1:
			...
			break;
		case 2:
			fdc_data((adf_rsector + (adf_size[adf_drive] !=
				    256)) ? 1 : 0);
			break;
		case 3:
			fdc_data((adf_size[adf_drive] ==
				256) ? 1 : ((adf_size[adf_drive] ==
				    512) ? 2 : 3));
			break;
		case 4:
			fdc_data(0);
...
//                        bem_debugf("Read addr - %i %i %i %i 1 0 0\n",adfdrive,adftrack,adfside,adfsector);
	...
	if (adf_informat && adf_f[adf_drive]) {
		...
		trackinfoa[adf_drive][adf_side][(adf_sector *
			adf_size[adf_drive]) + adf_readpos] = 0;
		adf_readpos++;
		if (adf_readpos == adf_size[adf_drive]) {
			adf_readpos = 0;
			adf_sector++;
