...
uint32_t       *usrregs[16], userregs[16], superregs[16], fiqregs[16],
    irqregs[16];
uint32_t        armregs[16];
	...
	for (c = 0; c < 16; c++) {
		for (d = 0; d < 16; d++) {
			...
			case 11:
				...
				break;
			case 12:
				 /*GT*/ exec = (!ZFSET && (NFSET == VFSET));
				break;
			case 13:
				 /*LE*/ exec = (ZFSET || (NFSET != VFSET));
				break;
			case 14:
				 /*AL*/ exec = 1;
				break;
...
void arm_init()
{
	...
	char            fn[256];
	if (!armrom)
		armrom = (uint32_t *) malloc(0x4000);
	if (!armram)
		armram = (uint32_t *) malloc(0x400000);
...
uint8_t readarmb(uint32_t addr)
{
	...
	if ((addr >= 0x3000000) && (addr < 0x3004000))
		return armromb[addr & 0x3FFF];
	return 0xFF;
...
{
	if (addr < 0x400000)
		return armram[addr >> 2];
	if (addr < 0x400010)
		return 0xFFFFFFFF;
	if ((addr >= 0x3000000) && (addr < 0x3004000))
		return armrom[(addr & 0x3FFC) >> 2];
	return 0xFFFFFFFF;
...
//                cycles--;
	...
	temp = armregs[RM];
//        if (RM==15)        temp+=4;
	if (opcode & 0x100000 && shiftamount)
		armregs[15] &= ~CFLAG;
	switch (shiftmode) {
	case 0:
		 /*LSL*/ if (!shiftamount)
	...
	case 1:
		 /*LSR*/ if (!shiftamount && !(opcode & 0x10)) {
			shiftamount = 32;
		}
		if (!shiftamount)
			return temp;
		if (shiftamount == 32) {
			if (temp & 0x80000000 && opcode & 0x100000)
				armregs[15] |= CFLAG;
	...
	case 2:
		...
		if (opcode & 0x100000) {
			if (((int) temp >> (shiftamount - 1)) & 1)
				armregs[15] |= CFLAG;
		}
		return (int) temp >> shiftamount;

	case 3:
		 /*ROR*/ if (opcode & 0x100000)
			armregs[15] &= ~CFLAG;
		if (!shiftamount && !(opcode & 0x10)) {
			if (opcode & 0x100000 && temp & 1)
				armregs[15] |= CFLAG;
		...
		if (!(shiftamount & 0x1F)) {
			if (opcode & 0x100000 && temp & 0x80000000)
				armregs[15] |= CFLAG;
			return temp;
		}
		if (opcode & 0x100000) {
			if (((temp >> shiftamount) | (temp << (32 -
					shiftamount))) & 0x80000000)
				armregs[15] |= CFLAG;
		}
...
//        if (RM==15) temp+=4;
	...
	case 2:
		 /*ASR*/ if (!shiftamount && !(opcode & 0x10))
			shiftamount = 32;
		if (shiftamount >= 32) {
			if (temp & 0x80000000)
				return 0xFFFFFFFF;
	...
	case 3:
		 /*ROR*/ if (!shiftamount && !(opcode & 0x10))
			return (((cflag) ? 1 : 0) << 31) | (temp >> 1);
		if (!shiftamount)
			return temp;
		return (temp >> shiftamount) | (temp << (32 - shiftamount));
...
//                                        }
			...
			case 0x0B:	/*ADCS reg */
				if (RD == 15) {
					...
					templ = shift2(opcode);
					armregs[15] =
					    GETADDR(RN) + templ + templ2 + 4;
					refillpipeline();
				} else {
					templ2 = CFSET;
					templ = shift2(opcode);
					setadc(GETADDR(RN), templ,
					    GETADDR(RN) + templ + templ2);
					armregs[RD] =
					    GETADDR(RN) + templ + templ2;
				}
...
//                                                bem_debugf("TEQ %08X %08X\n",GETADDR(RN),rotate(opcode));
			...
			case 0x35:	/*CMP imm */
				if (RD == 15) {
					...
					setsub(GETADDR(RN), rotate2(opcode),
					    GETADDR(RN) - rotate2(opcode));
				tubecycles--;
				break;
			...
			case 0x37:	/*CMN imm */
				if (RD == 15) {
					...
					setadd(GETADDR(RN), rotate2(opcode),
					    GETADDR(RN) + rotate2(opcode));
				tubecycles--;
				break;
			...
			case 0x58:
			case 0x70:
			case 0x78:	/*STR Rd,[Rn,offset] */
			case 0x52:
			case 0x5A:
			case 0x72:
			case 0x7A:	/*STR Rd,[Rn,offset]! */
				if (opcode & 0x2000000)
					addr2 = shift2(opcode);
				else
					addr2 = opcode & 0xFFF;
			...
			case 0x5C:
			case 0x74:
			case 0x7C:	/*STRB Rd,[Rn,offset] */
			case 0x56:
			case 0x5E:
			case 0x76:
			case 0x7E:	/*STRB Rd,[Rn,offset]! */
				if (opcode & 0x2000000)
					addr2 = shift2(opcode);
				else
					addr2 = opcode & 0xFFF;
...
#define LDMallS()       mask=1; \
			...
			case 0x80:
			 /*STMDA*/ case 0x82:	/*STMDA ! */
			case 0x90:
			 /*STMDB*/ case 0x92:	/*STMDB ! */
				addr =
				...
				if (opcode & 0x200000)
					armregs[RN] -=
					    countbits(opcode & 0xFFFF);
				STMall()
				    tubecycles--;
				break;
			case 0x88:
			 /*STMIA*/ case 0x8A:	/*STMIA ! */
			case 0x98:
			 /*STMIB*/ case 0x9A:	/*STMIB ! */
				addr = armregs[RN];
				if (opcode & 0x1000000)
					addr += 4;
				STMfirst();
			...
			case 0x81:
			 /*LDMDA*/ case 0x83:	/*LDMDA ! */
			case 0x91:
			 /*LDMDB*/ case 0x93:	/*LDMDB ! */
				addr =
				    armregs[RN] - countbits(opcode & 0xFFFF);
//                                        bem_debugf("LDMDB %08X\n",addr);
				...
				break;
			case 0x89:
			 /*LDMIA*/ case 0x8B:	/*LDMIA ! */
			case 0x99:
			 /*LDMIB*/ case 0x9B:	/*LDMIB ! */
				addr = armregs[RN];
				if (opcode & 0x1000000)
					addr += 4;
				if (opcode & 0x200000)
		...
		if (databort | armirq | tube_irq) {
			if (databort == 1) {	/*Data abort */
				...
				refillpipeline();
//                                        bem_debug("FIQ\n");
			} else if ((armirq & 1) && !(armregs[15] & 0x8000000))
				/*IRQ*/ {
				templ = armregs[15];
				armregs[15] |= 3;
				updatemode(IRQ);
