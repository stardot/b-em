...
void gui_update()
{
	...
	for (x = 0; x < 5; x++)
		tubemenu[x].flags =
		    (selecttube ==
		    (intptr_t) tubemenu[x].dp) ? D_SELECTED : 0;
#else
	for (x = 0; x < 4; x++)
		tubemenu[x].flags =
		    (selecttube ==
	...
	for (x = 0; x < 5; x++)
		tubespdmenu[x].flags =
		    (tube_6502_speed ==
		    (intptr_t) tubespdmenu[x].dp) ? D_SELECTED : 0;
	...
	for (x = 0; x < 3; x++)
		bordersmenu[x].flags =
		    (vid_fullborders ==
		    (intptr_t) bordersmenu[x].dp) ? D_SELECTED : 0;
	...
	for (x = 0; x < 5; x++)
		waveformmenu[x].flags =
		    (curwave ==
		    (intptr_t) waveformmenu[x].dp) ? D_SELECTED : 0;
	for (x = 0; x < 14; x++)
		sidtypemenu[x].flags =
		    (cursid == (intptr_t) sidtypemenu[x].dp) ? D_SELECTED : 0;
	...
	for (x = 0; x < 3; x++)
		ddvolmenu[x].flags =
		    (ddnoise_vol ==
		    (intptr_t) ddvolmenu[x].dp) ? D_SELECTED : 0;
	keymenu[1].flags = (keyas) ? D_SELECTED : 0;
	mousemenu[0].flags = (mouse_amx) ? D_SELECTED : 0;
	for (x = 0; x < 10; x++)
		speedmenu[x].flags =
		    (emuspeed == (intptr_t) speedmenu[x].dp) ? D_SELECTED : 0;
...
int gui_loadss()
{
	...
	ret =
	    file_select_ex("Please choose a save state", tempname, "SNP", 260,
	    xsize, ysize);
	if (ret) {
...
int gui_savess()
{
	...
	int             xsize = windx - 32, ysize = windy - 16;
	if (curtube != -1) {
		alert(NULL, "Second processor save states not supported yet.",
		    NULL, "&OK", NULL, 0, 0);
		return D_CLOSE;
	}
	memcpy(tempname, discfns[0], 260);
...
static int gui_load_drive(int drive, const char *prompt)
{
	...
	ret =
	    file_select_ex(prompt, tempname, "SSD;DSD;IMG;ADF;ADL;FDI", 260,
	    xsize, ysize);
	if (ret) {
		disc_close(drive);
...
int gui_autoboot()
{
	...
	if ((ret =
		gui_load_drive(0,
		    "Please choose a disc image to autoboot in drive 0/2"))) {
		main_reset();
		autoboot = 150;
	}
	gui_update();
...
int gui_vdfs_root()
{
	...
	ret =
	    file_select_ex("Please select VDFS root directory", tempname,
	    NULL, 260, xsize, ysize);
	if (ret)
...
int gui_loadt()
{
	...
	ret =
	    file_select_ex("Please choose a tape image", tempname, "UEF;CSW",
	    260, xsize, ysize);
	if (ret) {
		tape_close();
...
MENU            sidtypemenu[15] = {
	...
	{"6581R3 0486S", gui_sidtype, NULL, 0,
	    (void *) SID_MODEL_6581R3_0486S},
	{"6581R3 3984", gui_sidtype, NULL, 0, (void *) SID_MODEL_6581R3_3984},
	{"6581R4AR 3789", gui_sidtype, NULL, 0,
	    (void *) SID_MODEL_6581R4AR_3789},
	{"6581R3 4485", gui_sidtype, NULL, 0, (void *) SID_MODEL_6581R3_4485},
	{"6581R4 1986S", gui_sidtype, NULL, 0,
	    (void *) SID_MODEL_6581R4_1986S},
	{"8580R5 3691", gui_sidtype, NULL, 0, (void *) SID_MODEL_8580R5_3691},
	{"8580R5 3691 + digi boost", gui_sidtype, NULL, 0,
	    (void *) SID_MODEL_8580R5_3691D},
	{"8580R5 1489", gui_sidtype, NULL, 0, (void *) SID_MODEL_8580R5_1489},
	{"8580R5 1489 + digi boost", gui_sidtype, NULL, 0,
	    (void *) SID_MODEL_8580R5_1489D},
	{NULL, NULL, NULL, 0, NULL}
};
...
int gui_scrshot()
{
	...
	tempname[0] = 0;
	ret =
	    file_select_ex("Please enter filename", tempname, "BMP", 260,
	    xsize, ysize);
	if (ret) {
