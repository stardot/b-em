...
{
	if (OS01)
		level = !level;	/*OS 0.1 programs CA2 to interrupt on negative edge and expects the keyboard to still work */
...
static void sysvia_update_sdb()
{
	...
	keycol = sdbval & 0xF;
	key_update();
	if (!(IC32 & 8) && !bbckey[keycol][keyrow])
		sdbval &= 0x7f;
}
...
static void sysvia_write_IC32(uint8_t val)
{
	...
	if ((IC32 & 0xC0) != (oldIC32 & 0xC0)) {
		if (!(IC32 & 0x40))
			temp |= KB_CAPSLOCK_FLAG;
		if (!(IC32 & 0x80))
			temp |= KB_SCROLOCK_FLAG;
	}
	if (MASTER && !compactcmos)
