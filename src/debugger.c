...
void _debugthread(PVOID pvoid)
{
	...
	dhwnd = CreateWindowEx(0,	/* Extended possibilites for variation */
	...
	while (debugon) {
		...
		SetDIBitsToDevice(memDC, 0, 0, 256, 256, 0, 0, 0, 256, usdat,
...
LRESULT CALLBACK DebugWindowProcedure(HWND hwnd, UINT message, WPARAM wParam,
    LPARAM lParam)
{
...
static uint16_t debug_disassemble(uint16_t addr, void (*out) (const char *s,
	size_t len))
{
	uint8_t         op, p1, p2;
	uint16_t        temp;
	...
	case IMM:
		...
		sptr +=
		    sprintf(sptr, "%02X    %s #%02X     ", p1, op_name, p1);
		break;
	case ZP:
		p1 = debug_readmem(addr++);
		sptr +=
	...
	case ZPX:
		p1 = debug_readmem(addr++);
		sptr +=
		    sprintf(sptr, "%02X    %s %02X,X    ", p1, op_name, p1);
		break;
	case ZPY:
		p1 = debug_readmem(addr++);
		sptr +=
		    sprintf(sptr, "%02X    %s %02X,Y    ", p1, op_name, p1);
		break;
	case IND:
		p1 = debug_readmem(addr++);
		sptr +=
	...
	case ABS:
		...
		sptr +=
		    sprintf(sptr, "%02X %02X %s %02X%02X    ", p1, p2,
		    op_name, p2, p1);
		break;
	case ABSX:
		p1 = debug_readmem(addr++);
		p2 = debug_readmem(addr++);
		sptr +=
		    sprintf(sptr, "%02X %02X %s %02X%02X,X  ", p1, p2,
		    op_name, p2, p1);
		break;
	case ABSY:
		p1 = debug_readmem(addr++);
		p2 = debug_readmem(addr++);
		sptr +=
		    sprintf(sptr, "%02X %02X %s %02X%02X,Y  ", p1, p2,
		    op_name, p2, p1);
		break;
	case IND16:
		p1 = debug_readmem(addr++);
	...
	case BRA:
		...
		sptr +=
		    sprintf(sptr, "%02X    %s %04X    ", p1, op_name, temp);
		break;
	}
	out(s, sptr - s);
	return addr;
...
void debug_read(uint16_t addr)
{
	...
	for (c = 0; c < 8; c++) {
		...
		if (watchr[c] == addr)
			debug_outf
			    ("    Read from %04X - A=%02X X=%02X Y=%02X PC=%04X\n",
			    addr, a, x, y, pc);
	}
}
...
void debug_write(uint16_t addr, uint8_t val)
{
	...
	for (c = 0; c < 8; c++) {
		...
		if (watchw[c] == addr)
			debug_outf
			    ("    Write %02X to %04X - A=%02X X=%02X Y=%02X PC=%04X\n",
			    val, addr, a, x, y, pc);
	}
}
...
void debugger_do()
{
	...
	while (1) {
		...
		case 'B':
			if (!strncasecmp(ins, "breakr", 6)) {
				...
				for (c = 0; c < 8; c++) {
					if (breakr[c] != -1)
						debug_outf
						    ("    Read breakpoint %i : %04X\n",
						    c, breakr[c]);
				}
				for (c = 0; c < 8; c++) {
					if (breakw[c] != -1)
						debug_outf
						    ("    Write breakpoint %i : %04X\n",
						    c, breakw[c]);
				}
		...
		case 'D':
			if (params)
				sscanf(&ins[d], "%X",
				    (unsigned int *) &debug_disaddr);
			for (c = 0; c < 12; c++) {
				debug_out("    ", 4);
				debug_disaddr =
				    debug_disassemble(debug_disaddr,
				    debug_out);
				debug_out("\n", 1);
			}
			break;
		...
		case 'M':
			if (params)
				sscanf(&ins[d], "%X",
				    (unsigned int *) &debug_memaddr);
			for (c = 0; c < 16; c++) {
				debug_outf("    %04X : ", debug_memaddr);
				for (d = 0; d < 16; d++)
					debug_outf("%02X ",
					    debug_readmem(debug_memaddr + d));
				debug_out("  ", 2);
				dptr = dump;
				for (d = 0; d < 16; d++) {
					temp =
		...
		case 'R':
			if (params) {
				if (!strncasecmp(&ins[d], "sysvia", 6)) {
					debug_outf
					    ("    System VIA registers :\n");
					debug_outf
					    ("    ORA  %02X ORB  %02X IRA %02X IRB %02X\n",
					    sysvia.ora, sysvia.orb,
					    sysvia.ira, sysvia.irb);
					debug_outf
					    ("    DDRA %02X DDRB %02X ACR %02X PCR %02X\n",
					    sysvia.ddra, sysvia.ddrb,
					    sysvia.acr, sysvia.pcr);
					debug_outf
					    ("    Timer 1 latch %04X   count %04X\n",
					    sysvia.t1l / 2,
					    (sysvia.t1c / 2) & 0xFFFF);
					debug_outf
					    ("    Timer 2 latch %04X   count %04X\n",
					    sysvia.t2l / 2,
					    (sysvia.t2c / 2) & 0xFFFF);
					debug_outf("    IER %02X IFR %02X\n",
					    sysvia.ier, sysvia.ifr);
				} else if (!strncasecmp(&ins[d], "uservia",
					7)) {
					debug_outf
					    ("    User VIA registers :\n");
					debug_outf
					    ("    ORA  %02X ORB  %02X IRA %02X IRB %02X\n",
					    uservia.ora, uservia.orb,
					    uservia.ira, uservia.irb);
					debug_outf
					    ("    DDRA %02X DDRB %02X ACR %02X PCR %02X\n",
					    uservia.ddra, uservia.ddrb,
					    uservia.acr, uservia.pcr);
					debug_outf
					    ("    Timer 1 latch %04X   count %04X\n",
					    uservia.t1l / 2,
					    (uservia.t1c / 2) & 0xFFFF);
					debug_outf
					    ("    Timer 2 latch %04X   count %04X\n",
					    uservia.t2l / 2,
					    (uservia.t2c / 2) & 0xFFFF);
					debug_outf("    IER %02X IFR %02X\n",
					...
					debug_outf("    Index=%i\n", crtc_i);
					debug_outf
					    ("    R0 =%02X  R1 =%02X  R2 =%02X  R3 =%02X  R4 =%02X  R5 =%02X  R6 =%02X  R7 =%02X  R8 =%02X\n",
					    crtc[0], crtc[1], crtc[2],
					    crtc[3], crtc[4], crtc[5],
					    crtc[6], crtc[7], crtc[8]);
					debug_outf
					    ("    R9 =%02X  R10=%02X  R11=%02X  R12=%02X  R13=%02X  R14=%02X  R15=%02X  R16=%02X  R17=%02X\n",
					    crtc[9], crtc[10], crtc[11],
					    crtc[12], crtc[13], crtc[14],
					    crtc[15], crtc[16], crtc[17]);
					debug_outf
					    ("    VC=%i SC=%i HC=%i MA=%04X\n",
					    vc, sc, hc, ma);
				}
				if (!strncasecmp(&ins[d], "vidproc", 7)) {
					...
					debug_outf
					    ("     0=%01X   1=%01X   2=%01X   3=%01X   4=%01X   5=%01X   6=%01X   7=%01X\n",
					    ula_palbak[0], ula_palbak[1],
					    ula_palbak[2], ula_palbak[3],
					    ula_palbak[4], ula_palbak[5],
					    ula_palbak[6], ula_palbak[7]);
					debug_outf
					    ("     8=%01X   9=%01X  10=%01X  11=%01X  12=%01X  13=%01X  14=%01X  15=%01X\n",
					    ula_palbak[8], ula_palbak[9],
					    ula_palbak[10], ula_palbak[11],
					    ula_palbak[12], ula_palbak[13],
					    ula_palbak[14], ula_palbak[15]);
				}
				if (!strncasecmp(&ins[d], "sound", 5)) {
					debug_outf("    Sound registers :\n");
					debug_outf
					    ("    Voice 0 frequency = %04X   volume = %i  control = %02X\n",
					    sn_latch[0] >> 6, sn_vol[0],
					    sn_noise);
					debug_outf
					    ("    Voice 1 frequency = %04X   volume = %i\n",
					    sn_latch[1] >> 6, sn_vol[1]);
					debug_outf
					    ("    Voice 2 frequency = %04X   volume = %i\n",
					    sn_latch[2] >> 6, sn_vol[2]);
					debug_outf
					    ("    Voice 3 frequency = %04X   volume = %i\n",
					    sn_latch[3] >> 6, sn_vol[3]);
				}
			} else {
				debug_outf("    6502 registers :\n");
				debug_outf
				    ("    A=%02X X=%02X Y=%02X S=01%02X PC=%04X\n",
				    a, x, y, s, pc);
		...
		case 'T':
			...
			if (params) {
				...
				if ((trace_fp = fopen(&ins[d], "a")))
					...
					debug_outf
					    ("Unable to open trace file '%s' for append: %s\n",
		...
		case 'W':
			if (!strncasecmp(ins, "watchr", 6)) {
				...
				for (c = 0; c < 8; c++)
					if (watchr[c] != -1)
						debug_outf
						    ("    Read watchpoint %i : %04X\n",
						    c, watchr[c]);
				for (c = 0; c < 8; c++)
					if (watchw[c] != -1)
						debug_outf
						    ("    Write watchpoint %i : %04X\n",
						    c, watchw[c]);
			}
