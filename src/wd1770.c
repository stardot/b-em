...
static void wd1770_begin_seek(const char *cmd_desc, int target)
{
    log_debug("wd1770: begin %s to track %d", cmd_desc, target);
    if (target < 0) {
        log_debug("wd1770: refusing to step out from track 0");
        target = 0;
    }
...
static void wd1770_cmd(uint8_t val)
{
    unsigned cmd = val >> 4;
    if (wd1770.status & 1 && cmd != 0xD) {
        log_warn("wd1770: attempt to write %s register with %02X when device busy", "command", val);
        return;
    }
    nmi &= ~1;
    if (cmd != 0xD)/* && !(val&8)) */
    ...
    {
    case 0x0: /*Restore*/
        wd1770_begin_seek("restore", 0);
        break;

    case 0x1: /*Seek*/
        wd1770_begin_seek("seek", wd1770.data);
        break;

    case 0x2:
    case 0x3: /*Step*/
        wd1770_begin_seek("step", wd1770.curtrack + wd1770.stepdir);
        break;

    case 0x4:
    case 0x5: /*Step in*/
        wd1770.stepdir = 1;
        wd1770_begin_seek("step in", wd1770.curtrack+1);
        break;

    case 0x6:
    case 0x7: /*Step out*/
        wd1770.stepdir = -1;
        wd1770_begin_seek("step out", wd1770.curtrack-1);
        break;
    ...
    case 0xE:
        log_debug("wd1770: read track command (%02X) not implemented",val);
...
static void write_1770(uint16_t addr, uint8_t val)
{
    ...
    case 1: // Track register
        if (wd1770.status & 0x01)
            log_warn("wd1770: attempt to write %s register with %02X when device busy", "track", val);
        else {
    ...
    case 2: // Sector register
        if (wd1770.status & 0x01)
            log_warn("wd1770: attempt to write %s register with %02X when device busy", "sector", val);
        else {
...
void wd1770_finishread()
{
    log_debug("wd1770: data i/o finished, nmi=%02X", nmi);
    wd1770.status &= 0x83;
    fdc_time = 100;
}
...
void wd1770_datacrcerror()
{
    wd1770_fault(0x08, "data CRC error");
}
