...
static void scan_entry(vdfs_ent_t * ent)
{
	...
	ptr = host_file_path =
	    malloc(strlen(host_dir_path) + strlen(ent->host_fn) + 6);
	...
	*ptr = '\0';
	...
	free(host_file_path);
	bem_debugf
	    ("vdfs: scan_entry: acorn=%s, host=%s, attr=%04X, load=%08X, exec=%08X\n",
	    ent->acorn_fn, ent->host_fn, ent->attribs, ent->load_addr,
	    ent->exec_addr);
}
			...
			if (*ptr != ent) {
				...
				if (name_len < (MAX_FILE_NAME - 2)) {
					...
					ent->acorn_fn[MAX_FILE_NAME - 2] =
					    '~';
					ent->acorn_fn[MAX_FILE_NAME - 1] =
					    seq_ch;
				}
				ptr =
				    tsearch(ent, &dir->acorn_tree,
				    acorn_comp);
				while (*ptr != ent) {
					if (seq_ch == '9')
						seq_ch = 'A';
						...
						seq_ch++;
					if (name_len < (MAX_FILE_NAME - 2))
						ent->acorn_fn[name_len + 1] =
						    seq_ch;
					else
						ent->acorn_fn[MAX_FILE_NAME -
						    1] = seq_ch;
					ptr =
					    tsearch(ent, &dir->acorn_tree,
					    acorn_comp);
				...
				bem_debugf
				    ("vdfs: new_entry: unique name %s used\n",
				    ent->acorn_fn);
			}
	...
	if ((dp = opendir(dir->host_path))) {
		...
		while ((dep = readdir(dp))) {
			if (*(dep->d_name) != '.') {
				...
				if ((ptr =
					tfind(&key, &dir->host_tree,
					    host_comp))) {
					ent = *ptr;
					ent->attribs &= ~ATTR_DELETED;
					scan_entry(ent);
				} else if ((ent =
					new_entry(dir, dep->d_name)))
					count++;
				else {
					count = -1;
					break;
		...
		if (count >= 0) {
			...
			if ((dir->cat_tab =
				malloc(sizeof(vdfs_ent_t *) * count))) {
				cat_ptr = dir->cat_tab;
...
static vdfs_ent_t *find_file(uint16_t fn_addr, vdfs_ent_t * key,
    vdfs_ent_t * ent, uint16_t * tail_addr)
{
	int             i, ch;
	char           *fn_ptr;
	vdfs_ent_t    **ptr;
		...
		for (;;) {
			...
			*fn_ptr = '\0';
			...
			if (key->acorn_fn[0] == '$'
			    && key->acorn_fn[1] == '\0')
				ent = &root_dir;
			else if (key->acorn_fn[0] == '^'
			    && key->acorn_fn[1] == '\0')
				ent = ent->parent;
			else if ((ptr =
				tfind(key, &ent->acorn_tree, acorn_comp)))
				ent = *ptr;
			else
...
static void write_back(vdfs_ent_t * ent)
{
	...
	if ((fp = fopen(host_file_path, "wt"))) {
		fprintf(fp, "%-*s %08X %08X %08X\n", MAX_FILE_NAME,
		    ent->acorn_fn, ent->load_addr, ent->exec_addr,
		    ent->length);
		fclose(fp);
	} else
		bem_warnf
		    ("vdfs: unable to create INF file '%s' for '%s': %s\n",
...
static FILE    *open_file(vdfs_ent_t * ent, const char *mode)
{
	...
	host_dir_path = ent->parent->host_path;
	ptr = host_file_path =
	    malloc(strlen(host_dir_path) + strlen(ent->host_fn) + 2);
	if (host_dir_path[0] != '.' || host_dir_path[1] != '\0') {
		ptr = stpcpy(ptr, host_dir_path);
		*ptr++ = '/';
	}
...
static FILE    *getfp(int channel)
{
	...
	if (channel >= MIN_CHANNEL && channel < MAX_CHANNEL) {
		if ((fp = vdfs_chan[channel - MIN_CHANNEL].fp))
			...
			bem_debugf("vdfs: attempt to use closed channel %d",
			    channel);
	} else
...
static inline void osfind()
{
	...
	if (a == 0) {		// close file.
		...
		if ((fp = open_file(ent, mode))) {
			...
			bem_warnf
			    ("vdfs: osfind: unable to open file '%s' in mode '%s': %s\n",
...
static inline void osgbpb()
{
	...
	case 0x08:		// list files in current directory in Acorn format.
		...
		if (seq_ptr < cur_dir->cat_size) {
			...
			n = readmem32(pb + 5);
			bem_debugf
			    ("vdfs: seq_ptr=%d, writing max %d entries starting %04X\n",
			    seq_ptr, n, mem_ptr);
			do {
	...
	default:
		bem_debugf
		    ("vdfs: osgbpb unimplemented for a=%d, x=%d, y=%d\n", a,
		    x, y);
		bem_debugf
		    ("vdfs: osgbpb pb.channel=%d, data=%04X num=%04X, ptr=%04X\n",
		    readmem(pb), readmem32(pb + 1), readmem32(pb + 6),
		    readmem32(pb + 9));
	}
	p.c = status;
}
...
static inline void osargs()
{
	...
	if (y == 0) {
		...
		case 0xff:
			...
			break;
		default:
			bem_debugf("vdfs: osargs: y=0, a=%d not implemented",
			    a);
		}
	} else if ((fp = getfp(y))) {
		switch (a) {
		...
		default:
			bem_debugf
			    ("vdfs: osargs: unrecognised function code a=%d for channel y=%d",
			    a, y);
		}
	}
}
...
static inline void osfile()
{
	...
	default:
		bem_debugf
		    ("vdfs: osfile unimplemented for a=%d, x=%d, y=%d\n", a,
		    x, y);
	}
...
static void srp_tail(uint16_t addr, uint8_t flag, uint16_t fnaddr,
...
static int16_t swr_calc_addr(uint8_t flags, uint32_t * st_ptr, uint16_t romid)
{
	...
	if (flags & 0x40) {
		...
		bem_debugf
		    ("vdfs: swr_calc_addr: pseudo addr bank=%02d, start=%04x\n",
		    romid, start);
	} else {
		...
		bem_debugf
		    ("vdfs: exec_sram: abs addr bank=%02d, start=%04x\n",
		    romid, start);
		start -= 0x8000;
	}
	*st_ptr = start;
...
static inline void exec_swr_fs()
{
	...
	bem_debugf
	    ("vdfs: exec_swr_fs: flags=%02x, fn=%04x, romid=%02d, start=%04x, len=%04x\n",
	...
	if ((romid = swr_calc_addr(flags, &start, romid)) >= 0) {
		...
		if (flags & 0x80) {
			...
			if (len > 0) {
				if (ent) {
					if (ent->attribs & ATTR_IS_DIR)
						...
						fread(rom + romid * 0x4000 +
						    start, len, 1, fp);
						fclose(fp);
					} else {
						bem_warnf
						    ("vdfs: unable to load file '%s': %s\n",
						    ent->host_fn,
						    strerror(errno));
						adfs_hosterr(errno);
					}
			...
			if (len <= 16384) {
				if (!ent)
					ent =
					    add_new_file(cur_dir,
					    key.acorn_fn);
				if (ent) {
					if ((fp = open_file(ent, "wb"))) {
						fwrite(rom + romid * 0x4000 +
						    start, len, 1, fp);
						fclose(fp);
						load_add =
						    0xff008000 | (romid << 16)
						...
						bem_warnf
						    ("vdfs: unable to create file '%s': %s\n",
						    ent->host_fn,
						    strerror(errno));
				} else
					adfs_error(err_wont);
...
static void exec_swr_ram(uint8_t flags, uint16_t ram_start, uint16_t len,
    uint32_t sw_start, uint8_t romid)
{
	uint8_t        *rom_ptr;

	bem_debugf
	    ("vdfs: exec_swr_ram: flags=%02x, ram_start=%04x, len=%04x, sw_start=%04x, romid=%02d\n",
	...
	if ((addr = srp_start(addr, &ram_start))) {
		if ((addr = srp_length(addr, ram_start, &len))) {
			if ((addr = srp_start(addr, &sw_start))) {
				...
				exec_swr_ram(flags, ram_start, len, sw_start,
				    romid);
			}
		}
	}
...
static inline void dispatch(uint8_t value)
{
	...
	case 0xff:
		...
		break;
	default:
		bem_warnf("vdfs: function code %d not recognised\n", value);
	}
}
