...
void z80_dumpregs()
{
	...
	bem_debugf("%c%c%c%c%c%c   PC =%04X SP =%04X\n",
	    ...
	    (af.b.l & S_FLAG) ? 'S' : ' ', (af.b.l & C_FLAG) ? 'C' : ' ', pc,
	    sp);
...
//                                printf("IM 2\n");
			...
			case 0xA2:
				 /*INI*/ cycles += 5;
				temp = z80in(bc.w);	//z80_readmem(hl.w++);
				cycles += 3;
				z80_writemem(hl.w++, temp);
				af.b.l |= N_FLAG;
				bc.b.h--;
...
//                if (ins==11329) output=1;
		if (output)
			printf
			    ("%04X : %04X %04X %04X %04X %04X %04X %04X %04X %02X\n",
			    pc, af.w & 0xFF00, bc.w, de.w, hl.w, ix.w, iy.w,
			    sp, ir.w, opcode);
...
//                                printf("IM2 %04X %02X %02X %02X\n",ir.w,z80_readmem(ir.w|0xFF),z80_readmem(ir.w&~0xFF),z80_readmem((ir.w|0xFF)+1));
				pc = z80_readmem(0xFFFE) |
				    (z80_readmem(0xFFFF) << 8);
