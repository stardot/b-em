			...
			if ((dat_fp = fopen(dat_fn, "w"))) {
				if ((dsc_fp = fopen(dsc_fn, "w"))) {
					...
					cyl =
					    1 + ((sectors - 1) / (33 * 255));
					printf
					    ("size=%d, sectors=%d, cyl=%d\n",
					    size, sectors, cyl);
					geom[13] = cyl % 256;
					geom[14] = cyl / 256;
					geom[15] = 255;
					if (fwrite(geom, sizeof geom, 1,
						dsc_fp) != 1) {
						fprintf(stderr,
						    "hdfmt: unable to write to dsc file %s: %s\n",
					...
					status =
					    adfs_format(dat_fn, dat_fp,
					    sectors);
				} else {
					fprintf(stderr,
					    "hdfmt: unable to create dsc file %s: %s\n",
					    dsc_fn, strerror(errno));
					status = 2;
				}
				fclose(dat_fp);
				...
				fprintf(stderr,
				    "hdfmt: unable to create data file %s: %s\n",
