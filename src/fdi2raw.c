...
#else
static inline char *datalog(uae_u8 * src, int len)
{
	return "";
}
#endif
...
static uae_u8  *values_tree16(uae_u8 * stream, NODE * node)
{
	if (node->left == 0) {
		...
		uae_u8         *stream_temp =
		    values_tree16(stream, node->left);
...
static void fdi_decode(uae_u8 * stream, int size, uae_u8 * out)
{
	...
	while (sub_stream_shift) {
		...
		for (i = 0; i < size; i++) {
			...
			if (sign_extend) {
				if (sixteen_bit)
					v |= sign_extend16(current_node->v) <<
					    sub_stream_shift;
				else
					v |= sign_extend8(current_node->v) <<
					    sub_stream_shift;
			} else {
...
{
	int             i, bits =
	    (fdi->track_src[0] << 8) | fdi->track_src[1];
	uae_u8          b;
	fdi->track_src += 2;
	bem_debugf("s0a:bits=%d,data=%s", bits, datalog(fdi->track_src,
		(bits + 7) / 8));
	while (bits >= 8) {
...
{
	int             i, bits =
	    ((fdi->track_src[0] << 8) | fdi->track_src[1]) + 65536;
	uae_u8          b;
	fdi->track_src += 2;
	bem_debugf("s0b:bits=%d,data=%s", bits, datalog(fdi->track_src,
		(bits + 7) / 8));
	while (bits >= 8) {
		byte_add(fdi, *fdi->track_src++);
		bits -= 8;
...
static void s0c(FDI * fdi)
{
	...
	fdi->track_src += 2;
	bit_drop_next(fdi);
	bem_debugf("s0c:bits=%d,data=%s", bits, datalog(fdi->track_src,
		(bits + 7) / 8));
	while (bits >= 8) {
...
static void s0d(FDI * fdi)
{
	...
	fdi->track_src += 2;
	bit_drop_next(fdi);
	bem_debugf("s0d:bits=%d,data=%s", bits, datalog(fdi->track_src,
		(bits + 7) / 8));
	while (bits >= 8) {
		byte_mfm_add(fdi, *fdi->track_src++);
		bits -= 8;
...
//                      check_offset++;
		...
		if (chksum) {
			...
			if (slabel)
				bem_debug("(non-empty sector header)\n");
...
static void amiga_sector_header(FDI * fdi, uae_u8 * header, uae_u8 * data,
    int sector, int untilgap)
{
...
{
	bit_drop_next(fdi);
	bem_debugf("s20:header=%s,data=%s", datalog(fdi->track_src, 4),
	    datalog(fdi->track_src + 4, 16));
...
{
	bit_drop_next(fdi);
	bem_debugf("s22:sector=%d,untilgap=%d", fdi->track_src[0],
	    fdi->track_src[1]);
...
static void s24(FDI * fdi)
{
	...
	bem_debugf("s24:shift=%d,data=%s", shift, datalog(fdi->track_src,
		128 << shift));
...
{
	int             shift = *fdi->track_src++;
	bem_debugf("s25:shift=%d,crc=%s,data=%s", shift,
	    datalog(fdi->track_src, 4), datalog(fdi->track_src + 4,
		128 << shift));
...
static void ibm_sector_header(FDI * fdi, uae_u8 * data, uae_u8 * crc,
    int secnum, int pre)
{
	uae_u8          secbuf[5];
	uae_u8          crcbuf[2];
...
{
	bem_debugf("s17:header=%s,crc=%s", datalog(fdi->track_src, 4),
	    datalog(fdi->track_src + 4, 2));
...
{
	bem_debugf("s18:sector=%d,header=%s", *fdi->track_src,
	    datalog(fdi->track_src + 1, 4));
...
{
	int             shift = *fdi->track_src++;
	bem_debugf("s1a:shift=%d,data=%s", shift, datalog(fdi->track_src,
		128 << shift));
...
{
	int             shift = *fdi->track_src++;
	bem_debugf("s1b:shift=%d,crc=%s,data=%s", shift,
	    datalog(fdi->track_src + (128 << shift), 2),
	    datalog(fdi->track_src, 128 << shift));
	ibm_data(fdi, fdi->track_src, fdi->track_src + (128 << shift),
	    128 << shift);
...
static void fix_mfm_sync(FDI * fdi)
{
	...
	for (i = 0; i < fdi->mfmsync_offset; i++) {
		...
		mask2 = 1 << (7 - ((pos + 1) & 7));
		mask3 = 1 << (7 - (pos & 7));
		if (!(fdi->track_dst[off1] & mask1)
		    && !(fdi->track_dst[off2] & mask2))
...
static int handle_sectors_described_track(FDI * fdi)
{
	...
	do {
		...
		bem_debugf("%6.6X %6.6X %2.2X:",
		    (unsigned) (fdi->track_src - start_src + 0x200),
		    (fdi->out / 8), fdi->track_type);
		...
		if (fdi->track_src - fdi->track_src_buffer >=
		    fdi->track_src_len) {
			bem_debugf
			    ("source buffer overrun, previous type: %2.2X\n",
			    fdi->track_type);
			return -1;
		}
...
static uae_u8  *fdi_decompress(int pulses, uae_u8 * sizep, uae_u8 * src,
    int *dofree)
{
...
static void fdi2_decode(FDI * fdi, uint32_t totalavg, uae_u32 * avgp,
    uae_u32 * minp, uae_u32 * maxp, uae_u8 * idx, int maxidx,
    int *indexoffsetp, int pulses, int mfm)
{
	...
	while ((i < pulses) && ((idx[i] < maxidx)
		...
		|| (avgp[i] <
			...
			(standard_MFM_2_bit_cell_size / 4)))))
		i++;
	if (i == pulses) {
	...
	while (outstep < 2) {
		...
		if ((avg_size <
			(standard_MFM_8_bit_cell_size -
			    (pulse_limitval * standard_MFM_8_bit_cell_size /
				100)))
		    || (avg_size >
			(standard_MFM_8_bit_cell_size +
			    (pulse_limitval * standard_MFM_8_bit_cell_size /
		...
		if (adjusted_pulse >= ((avg_size * 3) + (avg_size / 4))) {
			if (adjusted_pulse <=
			    ((avg_size * 4) - (avg_size / 4))) {
				if ((2 * ((adjusted_pulse >> 2) - adjust)) <=
				    ((2 * avg_size) - (avg_size / 4)))
					real_size += 3;
				else
					real_size += 4;
			} else
			...
			if (adjusted_pulse >
			    ((avg_size * 3) - (avg_size / 4))) {
				real_size += 3;
			} else {
				if (adjusted_pulse >=
				    ((avg_size * 2) + (avg_size / 4))) {
					if ((2 * ((adjusted_pulse >> 2) -
						    adjust)) <
					    (avg_size + (avg_size / 4)))
						real_size += 2;
					else
		...
		while (pulse < ((avg_size / 4) - (avg_size / 16))) {
			...
			if (idx[i] >= maxidx) {	/* stable pulse */
				...
				if (jitter >= 0)
					...
					min_pulse -= jitter;
				if ((maxp[nexti] - avgp[nexti]) <
				    (avg_pulse - min_pulse))
					min_pulse =
					    avg_pulse - (maxp[nexti] -
					    avgp[nexti]);
				if ((avgp[nexti] - minp[nexti]) <
				    (max_pulse - avg_pulse))
					max_pulse =
					    avg_pulse + (avgp[nexti] -
					    minp[nexti]);
				...
				avg_pulse += jitter;
				if ((avg_pulse < min_pulse)
				    || (avg_pulse > max_pulse)) {
					bem_debugf
					    ("FDI: avg_pulse outside bounds! avg=%u min=%u max=%u\n",
					    avg_pulse, min_pulse, max_pulse);
					bem_debugf
					    ("FDI: avgp=%u (%u) minp=%u (%u) maxp=%u (%u) jitter=%ld i=%d ni=%d\n",
					    avgp[i], avgp[nexti], minp[i],
					    minp[nexti], maxp[i], maxp[nexti],
					    jitter, i, nexti);
				}
				if (avg_pulse < ref_pulse)
					bem_debugf
					    ("FDI: avg_pulse < ref_pulse! (%u < %u)\n",
					    avg_pulse, ref_pulse);
...
static void fdi2_celltiming(FDI * fdi, uint32_t totalavg, int bitoffset,
    uae_u16 * out)
{
	uae_u16        *pt2, *pt;
	double          avg_bit_len;
...
static int decode_lowlevel_track(FDI * fdi, int track,
    struct fdi_cache *cache)
{
	...
	len = 12;
	avgp =
	    (uae_u32 *) fdi_decompress(pulses, p1 + 0, p1 + len, &avg_free);
	...
	if (get_u24(p1 + 3) && get_u24(p1 + 6)) {
		minp =
		    (uae_u32 *) fdi_decompress(pulses, p1 + 3, p1 + len,
		    &min_free);
		len += get_u24(p1 + 3) & 0x3fffff;
		maxp =
		    (uae_u32 *) fdi_decompress(pulses, p1 + 6, p1 + len,
		    &max_free);
...
FDI            *fdi2raw_header(FILE *f)
{
	...
	if ((fdi->header[140] != 1 && fdi->header[140] != 2)
	    || (fdi->header[141] != 0 && !(fdi->header[140] == 2
		    && fdi->header[141] == 1))) {
		fdi_free(fdi);
		return NULL;
	}
...
int fdi2raw_loadrevolution_2(FDI * fdi, uae_u16 * mfmbuf,
    uae_u16 * tracktiming, int track, int *tracklength, int *indexoffsetp,
...
int fdi2raw_loadrevolution(FDI * fdi, uae_u16 * mfmbuf, uae_u16 * tracktiming,
    int track, int *tracklength, int mfm)
{
	return fdi2raw_loadrevolution_2(fdi, mfmbuf, tracktiming, track,
	    tracklength, 0, 0, mfm);
}

int fdi2raw_loadtrack(FDI * fdi, uae_u16 * mfmbuf, uae_u16 * tracktiming,
    int track, int *tracklength, int *indexoffsetp, int *multirev, int mfm)
{
	uae_u8         *p;
	int             outlen, i;
	struct fdi_cache *cache = &fdi->cache[track];
	...
	if (cache->lowlevel)
		return fdi2raw_loadrevolution_2(fdi, mfmbuf, tracktiming,
		    track, tracklength, indexoffsetp, multirev, mfm);

	fdi->err = 0;
	fdi->track_src_len =
	    fdi->track_offsets[track + 1] - fdi->track_offsets[track];
...
//      amiga_check_track (fdi);
	...
	if (outlen > 0) {
		if (cache->lowlevel)
			return fdi2raw_loadrevolution_2(fdi, mfmbuf,
			    tracktiming, track, tracklength, indexoffsetp,
			    multirev, mfm);
		*tracklength = fdi->out;
