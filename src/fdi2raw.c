...
static void fdi_free (void *p)
{
        ...
        size = ((int*)p)[-1];
        fdi_allocated -= size;
        bem_log(LOG_DEBUG, "%d freed (%d)", size, fdi_allocated);
        free ((int*)p - 1);
}
static void *fdi_malloc (int size)
{
	...
	((int*)p)[0] = size;
	fdi_allocated += size;
	bem_log(LOG_DEBUG, "%d allocated (%d)", size, fdi_allocated);
	return (int*)p + 1;
}
#else
...
{
	bem_log(LOG_DEBUG, "track %d: unknown track type 0x%2.2X", fdi->current_track, fdi->track_type);
//	return -1;
}
/* unsupported track */
#if 0
...
{
        bem_log(LOG_DEBUG, "track %d: unsupported track type 0x%02.2X", fdi->current_track, fdi->track_type);
//      return -1;
}
#endif
/* empty track */
...
{
	bem_log(LOG_DEBUG, "\ntrack %d: unknown sector described type 0x%2.2X", fdi->current_track, fdi->track_type);
	fdi->err = 1;
}
...
{
        bem_log(LOG_DEBUG, "\ntrack %d: unsupported sector described 0x%02.2X", fdi->current_track, fdi->track_type);
        fdi->err = 1;
}
#endif
...
static void add_mfm_sync_bit (FDI *fdi)
{
        ...
        if (fdi->out == 0) {
                bem_log(LOG_DEBUG, "illegal position for mfm sync bit, offset=%d",fdi->out);
                fdi->err = 1;
        }
        if (fdi->mfmsync_offset >= MAX_MFM_SYNC_BUFFER) {
                fdi->mfmsync_offset = 0;
                bem_log(LOG_DEBUG, "mfmsync buffer overflow");
                fdi->err = 1;
        }
        fdi->out++;
}
...
static void bit_add (FDI *fdi, int bit)
{
        ...
        if (fdi->out >= MAX_DST_BUFFER * 8) {
                bem_log(LOG_DEBUG, "destination buffer overflow");
                fdi->err = 1;
                fdi->out = 1;
        }
}
...
{
        if (fdi->nextdrop > 0) {
                bem_log(LOG_DEBUG, "multiple bit_drop_next() called");
        } else if (fdi->nextdrop < 0) {
                fdi->nextdrop = 0;
                bem_log(LOG_DEBUG, ":DNN:");
                return;
        }
        bem_log(LOG_DEBUG, ":DN:");
        fdi->nextdrop = 1;
}
...
{
        if (fdi->nextdrop) {
                bem_log(LOG_DEBUG, "bit_drop_next called before bit_dedrop");
        }
        fdi->nextdrop = -1;
        bem_log(LOG_DEBUG, ":BDD:");
}
...
static void s08(FDI *fdi)
{
	...
	bem_log(LOG_DEBUG, "s08:len=%d,data=%2.2X",bytes,byte);
...
static void s09(FDI *fdi)
{
	...
	if (bytes == 0) bytes = 256;
	bit_drop_next (fdi);
	bem_log(LOG_DEBUG, "s09:len=%d,data=%2.2X",bytes,byte);
...
static void s0a(FDI *fdi)
{
        ...
        uae_u8 b;
        fdi->track_src += 2;
        bem_log(LOG_DEBUG, "s0a:bits=%d,data=%s", bits, datalog(fdi->track_src, (bits + 7) / 8));
        while (bits >= 8) {
...
static void s0b(FDI *fdi)
{
        ...
        uae_u8 b;
        fdi->track_src += 2;
        bem_log(LOG_DEBUG, "s0b:bits=%d,data=%s", bits, datalog(fdi->track_src, (bits + 7) / 8));
        while (bits >= 8) {
                byte_add (fdi, *fdi->track_src++);
                bits -= 8;
        }
...
static void s0c(FDI *fdi)
{
        ...
        fdi->track_src += 2;
        bit_drop_next (fdi);
        bem_log(LOG_DEBUG, "s0c:bits=%d,data=%s", bits, datalog(fdi->track_src, (bits + 7) / 8));
        while (bits >= 8) {
...
static void s0d(FDI *fdi)
{
        ...
        fdi->track_src += 2;
        bit_drop_next (fdi);
        bem_log(LOG_DEBUG, "s0d:bits=%d,data=%s", bits, datalog(fdi->track_src, (bits + 7) / 8));
        while (bits >= 8) {
                byte_mfm_add (fdi, *fdi->track_src++);
                bits -= 8;
        }
...
//                      check_offset++;
                ...
                if (trackoffs + 1 > drvsec) {
                        bem_log(LOG_DEBUG, "illegal sector offset %d",trackoffs);
                        ok = 0;
                        mbuf = mbuf2;
                        continue;
                }
                if ((id >> 24) != 0xff) {
                        bem_log(LOG_DEBUG, "sector %d format type %02.2X?", trackoffs, id >> 24);
                        ok = 0;
                }
                chksum = odd ^ even;
                slabel = 0;
                ...
                if (((odd << 1) | even) != chksum) {
                        bem_log(LOG_DEBUG, "sector %d header crc error", trackoffs);
                        ok = 0;
                        mbuf = mbuf2;
                        continue;
                }
                bem_log(LOG_DEBUG, "sector %d header crc ok", trackoffs);
                if (((id & 0x00ff0000) >> 16) != (uae_u32)fdi->current_track) {
                        bem_log(LOG_DEBUG, "illegal track number %d <> %d",fdi->current_track,(id & 0x00ff0000) >> 16);
                        ok++;
                        mbuf = mbuf2;
                        continue;
                }
                ...
                mbuf += 256 * 2;
                if (chksum) {
                        bem_log(LOG_DEBUG, "sector %d data checksum error",trackoffs);
                        ok = 0;
                } else if (sectable[trackoffs]) {
                        bem_log(LOG_DEBUG, "sector %d already found?", trackoffs);
                        mbuf = mbuf2;
                } else {
                        bem_log(LOG_DEBUG, "sector %d ok",trackoffs);
                        if (slabel) bem_log(LOG_DEBUG, "(non-empty sector header)\n");
        ...
        for (i = 0; i < drvsec; i++) {
                if (!sectable[i]) {
                        bem_log(LOG_DEBUG, "sector %d missing", i);
                        ok = 0;
                }
        }
        return ok;
...
{
        bit_drop_next (fdi);
        bem_log(LOG_DEBUG, "s20:header=%s,data=%s", datalog(fdi->track_src, 4), datalog(fdi->track_src + 4, 16));
...
{
        bit_drop_next (fdi);
        bem_log(LOG_DEBUG, "s21:header=%s", datalog(fdi->track_src, 4));
...
{
        bit_drop_next (fdi);
        bem_log(LOG_DEBUG, "s22:sector=%d,untilgap=%d", fdi->track_src[0], fdi->track_src[1]);
...
{
        bem_log(LOG_DEBUG, "s23:data=%s", datalog (fdi->track_src, 512));
...
static void s24(FDI *fdi)
{
        ...
        bem_log(LOG_DEBUG, "s24:shift=%d,data=%s", shift, datalog (fdi->track_src, 128 << shift));
...
{
        int shift = *fdi->track_src++;
        bem_log(LOG_DEBUG, "s25:shift=%d,crc=%s,data=%s", shift, datalog (fdi->track_src, 4), datalog (fdi->track_src + 4, 128 << shift));
...
{
        s21 (fdi);
        bem_log(LOG_DEBUG, "s26:data=%s", datalog (fdi->track_src, 512));
        amiga_data (fdi, fdi->track_src);
        fdi->track_src += 512;
}
...
{
        s22 (fdi);
        bem_log(LOG_DEBUG, "s27:data=%s", datalog (fdi->track_src, 512));
        amiga_data (fdi, fdi->track_src);
        fdi->track_src += 512;
}
...
{
        bit_drop_next (fdi);
        bem_log(LOG_DEBUG, "s13:header=%s", datalog (fdi->track_src, 4));
...
{
        bem_log(LOG_DEBUG, "s14:header=%s", datalog (fdi->track_src, 4));
...
{
        bem_log(LOG_DEBUG, "s18:sector=%d,header=%s", *fdi->track_src, datalog (fdi->track_src + 1, 4));
...
{
        bem_log(LOG_DEBUG, "s19:data=%s", datalog (fdi->track_src , 512));
...
{
        int shift = *fdi->track_src++;
        bem_log(LOG_DEBUG, "s1a:shift=%d,data=%s", shift, datalog (fdi->track_src , 128 << shift));
...
{
        int shift = *fdi->track_src++;
        bem_log(LOG_DEBUG, "s1b:shift=%d,crc=%s,data=%s", shift, datalog (fdi->track_src + (128 << shift), 2), datalog (fdi->track_src , 128 << shift));
...
static int handle_sectors_described_track (FDI *fdi)
{
	...
	fdi->index_offset >>= 8;
	fdi->track_src += 3;
	bem_log(LOG_DEBUG, "sectors_described, index offset: %d",fdi->index_offset);

	do {
		fdi->track_type = *fdi->track_src++;
		bem_log(LOG_DEBUG, "%6.6X %6.6X %2.2X:",(unsigned)(fdi->track_src - start_src + 0x200), (fdi->out/8), fdi->track_type);
		decode_sectors_described_track[fdi->track_type](fdi);
#ifdef _DEBUG
		bem_log(LOG_DEBUG, " %d", fdi->out - oldout);
		oldout = fdi->out;
#endif
		if (fdi->out < 0 || fdi->err) {
		        bem_log(LOG_DEBUG, "\nin %ld bytes, out %d bits", (long)(fdi->track_src - fdi->track_src_buffer), fdi->out);
			return -1;
		}
		if (fdi->track_src - fdi->track_src_buffer >= fdi->track_src_len) {
			bem_log(LOG_DEBUG, "source buffer overrun, previous type: %2.2X", fdi->track_type);
			return -1;
		}
...
static void fdi2_decode (FDI *fdi, uint32_t totalavg, uae_u32 *avgp, uae_u32 *minp, uae_u32 *maxp, uae_u8 *idx, int maxidx, int *indexoffsetp, int pulses, int mfm)
{
        ...
        while ( (i < pulses) && ( (idx[i] < maxidx)
                ...
                || (avgp[i] < (standard_MFM_2_bit_cell_size - (standard_MFM_2_bit_cell_size / 4))) ) )
                        i++;
        if (i == pulses)  {
                bem_log(LOG_DEBUG, "No stable and long-enough pulse in track.");
                return;
        }
        i--;
        eodat = i;
...
static void fdi2_decode (FDI *fdi, uint32_t totalavg, uae_u32 *avgp, uae_u32 *minp, uae_u32 *maxp, uae_u8 *idx, int maxidx, int *indexoffsetp, int pulses, int mfm)
{
	...
	while ( (i < pulses) && ( (idx[i] < maxidx)
		...
		|| (minp[i] < (standard_MFM_2_bit_cell_size - (standard_MFM_2_bit_cell_size / 4))) ) )
			i++;
	if (i == pulses)  {
		bem_log(LOG_DEBUG, "FDI: No stable and long-enough pulse in track.");
		return;
	}
	nexti = i;
	eodat = i;
		...
		while (pulse < ((avg_size / 4) - (avg_size / 16))) {
			...
			if (idx[i] >= maxidx) { /* stable pulse */
				...
				if ((avg_pulse < min_pulse) || (avg_pulse > max_pulse)) {
					bem_log(LOG_DEBUG, "FDI: avg_pulse outside bounds! avg=%u min=%u max=%u", avg_pulse, min_pulse, max_pulse);
					bem_log(LOG_DEBUG, "FDI: avgp=%u (%u) minp=%u (%u) maxp=%u (%u) jitter=%ld i=%d ni=%d",
						avgp[i], avgp[nexti], minp[i], minp[nexti], maxp[i], maxp[nexti], jitter, i, nexti);
				}
				if (avg_pulse < ref_pulse)
					bem_log(LOG_DEBUG, "FDI: avg_pulse < ref_pulse! (%u < %u)", avg_pulse, ref_pulse);
...
static int decode_lowlevel_track (FDI *fdi, int track, struct fdi_cache *cache)
{
	...
	len = totalavg / 100000;
	bem_log(LOG_DEBUG, "totalavg=%u index=%d maxidx=%d weakbits=%d len=%d",
...
void fdi2raw_header_free (FDI *fdi)
{
        ...
        fdi_free (fdi);
#ifdef _DEBUG
        bem_log(LOG_DEBUG, "FREE: memory allocated %d", fdi_allocated);
#endif
}
...
FDI *fdi2raw_header(FILE *f)
{
...
#ifdef _DEBUG
        bem_log(LOG_DEBUG, "ALLOC: memory allocated %d", fdi_allocated);
#endif
        ...
        bem_log(LOG_DEBUG, "FDI version %d.%d", fdi->header[140], fdi->header[141]);
        bem_log(LOG_DEBUG, "last_track=%d rotation_speed=%d",fdi->last_track,fdi->rotation_speed);
...
int fdi2raw_loadrevolution_2 (FDI *fdi, uae_u16 *mfmbuf, uae_u16 *tracktiming, int track, int *tracklength, int *indexoffsetp, int *multirev, int mfm)
{
        ...
        bem_log(LOG_DEBUG, "track %d: nbits=%d avg len=%.2f weakbits=%d idx=%d",
...
int fdi2raw_loadtrack (FDI *fdi, uae_u16 *mfmbuf, uae_u16 *tracktiming, int track, int *tracklength, int *indexoffsetp, int *multirev, int mfm)
{
	...
	bem_log(LOG_DEBUG, "track %d: srclen: %d track_type: %2.2X, bitrate: %d",
