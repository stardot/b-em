...
int             params[][2] = {
	{0x35, 4}, {0x29, 1}, {0x2C, 0}, {0x3D, 1}, {0x3A, 2}, {0x13, 3},
	{0x0B, 3}, {0x1B, 3}, {0x1F, 3}, {0x23, 5}, {-1, -1}
};
...
//                                printf("Write special %02X\n",i8271.params[0]);
				...
				case 0x12:
					i8271.curtrack[0] = val;	/*printf("Write real track now %i\n",val); */
					break;
				case 0x17:
					break;	/*Mode register */
				case 0x1A:
					i8271.curtrack[1] = val;	/*printf("Write real track now %i\n",val); */
					break;
				case 0x23:
...
//        printf("Callback 8271 - command %02X\n",i8271.command);
	...
	case 0x0B:		/*Write */
		if (!i8271.phase) {
			...
			disc_writesector(curdrive, i8271.cursector,
			    i8271.params[0], (i8271.drvout & 0x20) ? 1 : 0,
			    0);
			i8271.phase = 1;
		...
		i8271.cursector++;
		disc_writesector(curdrive, i8271.cursector, i8271.params[0],
		    (i8271.drvout & 0x20) ? 1 : 0, 0);
		byte = 0;
		i8271.status = 0x8C;
		i8271.result = 0;
		i8271_NMI();
...
//                        printf("Re-seeking - track now %i %i\n",i8271.curtrack,i8271.realtrack);
			disc_readsector(curdrive, i8271.cursector,
			    i8271.params[0], (i8271.drvout & 0x20) ? 1 : 0,
			    0);
			i8271.phase = 1;
			return;
		...
		i8271.cursector++;
		disc_readsector(curdrive, i8271.cursector, i8271.params[0],
		    (i8271.drvout & 0x20) ? 1 : 0, 0);
		byte = 0;
		break;
...
//                        i8271.realtrack+=diff;
//                        disc_seek(0,i8271.realtrack);
			disc_readaddress(curdrive, i8271.params[0],
			    (i8271.drvout & 0x20) ? 1 : 0, 0);
			i8271.phase = 1;
			return;
		}
...
//                        printf("8271 : ID read done!\n");
	...
	case 0x23:		/*Format */
		...
		disc_format(curdrive, i8271.params[0],
		    (i8271.drvout & 0x20) ? 1 : 0, 0);
		i8271.phase = 2;
		break;
