#define _DEBUG
/* Tables and functions to enable disassembling the CINTCODE used
 * as an intermediate code for BCPL on the RCP implementation for
 * the BBC Micro.
 */
...
static void cintcode_hexnyb(char *dest, unsigned nyb)
{
    nyb &= 0x0f;
    if (nyb >= 10)
        *dest = 'A' + nyb - 10;
...
static void cintcode_hexbyte(char *dest, unsigned byte)
{
    cintcode_hexnyb(dest, byte >> 4);
    cintcode_hexnyb(dest+1, byte);
}

static void cintcode_hexword(char *dest, unsigned word)
{
    cintcode_hexnyb(dest, word >> 12);
    cintcode_hexnyb(dest+1, word >> 8);
    cintcode_hexnyb(dest+2, word >> 4);
    cintcode_hexnyb(dest+3, word);
}
...
static void cintcode_one_reg(cpu_debug_t *cpu, char *dest, int name, unsigned addr)
{
    ...
    dest[1] = name;
    dest[2] = '=';
    cintcode_hexbyte(dest+3, cpu->memread(addr+1));
    cintcode_hexbyte(dest+5, cpu->memread(addr));
}
...
static uint32_t cintcode_dis(cpu_debug_t *cpu, uint32_t addr, char *buf, size_t bufsize)
{
    ...
    assert(bufsize > 34);
    cintcode_hexword(buf, addr);
    buf[4] = ':';
    memset(buf+5, ' ', 29);
    addr |= 0x08000000;
    opcode = cpu->memread(addr++);
    cintcode_hexbyte(buf+6, opcode);
    ...
    switch(op->cc_am) {
        ...
        case CAM_BYTE:
            b1 = cpu->memread(addr++);
            cintcode_hexbyte(buf+9, b1);
            cintcode_hexbyte(buf+21, b1);
            break;
        case CAM_WORD:
            log_debug("cintcode: addr=%04X", addr);
            b1 = cpu->memread(addr++);
            b2 = cpu->memread(addr++);
            log_debug("cintcode: b1=%02X, b2=%02X", b1, b2);
            cintcode_hexbyte(buf+9, b1);
            cintcode_hexbyte(buf+12, b2);
            cintcode_hexbyte(buf+21, b2);
            cintcode_hexbyte(buf+23, b1);
            break;
        case CAM_BREL:
            b1 = cpu->memread(addr++);
            cintcode_hexbyte(buf+9, b1);
            cintcode_hexword(buf+21, addr + b1 - 0x80);
            break;
        case CAM_BIND:
            b1 = cpu->memread(addr++);
            cintcode_hexbyte(buf+9, b1);
            w = (addr + ((b1<<1)|1)) & 0xfffe;
            cintcode_hexword(buf+21, cpu->memread(w));
            break;
        case CAM_GLB2:
            global += 256;
            /* FALL THROUGH */
...
void cintcode_trace(cpu_debug_t *cpu, uint32_t cc_addr, FILE *trace_fp)
{
    char buf[80];
    cintcode_dis(cpu, cc_addr, buf, sizeof(buf));
