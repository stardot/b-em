...
/*                if (a==0xFEFB && temp==0xD)
                ...
                //bem_log(LOG_DEBUG, "Read TUBE  %04X %02X %04X\n",a,temp,pc);
                return temp;
        }
...
//        cycles-=2;
//        bem_log(LOG_DEBUG, "Reading %08X %i %08X %08X\n",a,tuberomin,w65816rom,w65816ram);
...
{
        a&=w65816mask;
//        if (a==0xFF) bem_log(LOG_DEBUG, "Write 00FF %02X %04X %i\n",v,pc,wins);
        cycles--;
        if ((a&~7)==0xFEF0)
        {
//                printf("Write control %04X %02X\n",a,v);
                switch (v&7)
                {
                        case 0: case 1: def=v&1; /*bem_log(LOG_DEBUG, "Default now %i\n",def); */break;
                        case 2: case 3: divider=(divider>>1)|((v&1)<<3); /*bem_log(LOG_DEBUG, "Divider now %i\n",divider);*/ break;
                        case 4: case 5: banking=(banking>>1)|((v&1)<<3); /*bem_log(LOG_DEBUG, "Banking now %i\n",banking);*/ break;
                        case 6: case 7: banknum=(banknum>>1)|((v&1)<<5); /*bem_log(LOG_DEBUG, "Banknum now %i\n",banknum);*/ break;
                }
                if (def || !(banking&4)) w65816mask=0xFFFF;
                else                     w65816mask=0x7FFFF;
//                bem_log(LOG_DEBUG, "Mask now %08X\n",w65816mask);
                return;
        }
        if ((a&~7)==0xFEF8)
        {
//                bem_log(LOG_DEBUG, "Write TUBE %04X %02X %04X\n",a,v,pc);
...
//        if (a>0xF000) bem_log(LOG_DEBUG, "Write %04X %02X %04X\n",a,v,pc);
...
//                printf("IRQ\n");
        ...
        else
        {
                ...
                p.i=1;
                p.d=0;
//                bem_log(LOG_DEBUG, "Emulation mode IRQ %04X\n",pc);
//                toutput=1;
//                dumpregs();
//                exit(-1);
        }
