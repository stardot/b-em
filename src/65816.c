...
//                printf("Write control %04X %02X\n",a,v);
                switch (v&7)
                {
                        case 0: case 1: def=v&1; /*log_debug("Default now %i\n",def); */break;
                        case 2: case 3: divider=(divider>>1)|((v&1)<<3); /*log_debug("Divider now %i\n",divider);*/ break;
                        case 4: case 5: banking=(banking>>1)|((v&1)<<3); /*log_debug("Banking now %i\n",banking);*/ break;
                        case 6: case 7: banknum=(banknum>>1)|((v&1)<<5); /*log_debug("Banknum now %i\n",banknum);*/ break;
                }
                if (def || !(banking&4)) w65816mask=0xFFFF;
                else                     w65816mask=0x7FFFF;
//                log_debug("Mask now %08X\n",w65816mask);
                return;
        }
        if ((a&~7)==0xFEF8)
        {
...
//                printf("IRQ\n");
        ...
        else
        {
                ...
                p.i=1;
                p.d=0;
//                log_debug("Emulation mode IRQ %04X\n",pc);
//                toutput=1;
//                dumpregs();
//                exit(-1);
        }
