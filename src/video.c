...
static uint8_t  crtc_mask[32] =
    { 0xFF, 0xFF, 0xFF, 0xFF, 0x7F, 0x1F, 0x7F, 0x7F, 0xF3, 0x1F, 0x7F, 0x1F,
...
/*Mode 7 (SAA5050)*/
static uint8_t  mode7_chars[96 * 160], mode7_charsi[96 * 160],
    mode7_graph[96 * 160], mode7_graphi[96 * 160], mode7_sepgraph[96 * 160],
    mode7_sepgraphi[96 * 160], mode7_tempi[96 * 120], mode7_tempi2[96 * 120];
...
void mode7_makechars()
{
	...
	for (c = 0; c < 960; c++) {
		...
		for (d = 0; d < 16; d++) {
			mode7_graph[offs2 + d] =
			    (int) (((float) teletext_graphics[offs1 +
				    x2] * (1.0 - x)) +
			    ((float) teletext_graphics[offs1 + x2 + 1] * x));
			mode7_sepgraph[offs2 + d] =
			    (int) (((float) teletext_separated_graphics[offs1
				    + x2] * (1.0 - x)) +
			    ((float) teletext_separated_graphics[offs1 + x2 +
				    1] * x));
			if (!d) {
				mode7_graph[offs2 + d] =
				    mode7_graphi[offs2 + d] =
				    teletext_graphics[offs1];
				mode7_sepgraph[offs2 + d] =
				    mode7_sepgraphi[offs2 + d] =
				    teletext_separated_graphics[offs1];
			} else if (d == 15) {
				mode7_graph[offs2 + d] =
				    mode7_graphi[offs2 + d] =
				    teletext_graphics[offs1 + 5];
				mode7_sepgraph[offs2 + d] =
				    mode7_sepgraphi[offs2 + d] =
				    teletext_separated_graphics[offs1 + 5];
			} else {
				mode7_graph[offs2 + d] =
				    mode7_graphi[offs2 + d] =
				    teletext_graphics[offs1 + x2];
				mode7_sepgraph[offs2 + d] =
				    mode7_sepgraphi[offs2 + d] =
				    teletext_separated_graphics[offs1 + x2];
			}
			x += (5.0 / 15.0);
			if (x >= 1.0) {
				x2++;
	...
	for (c = 0; c < 960; c++) {
		...
		for (d = 0; d < 16; d++) {
			mode7_chars[offs2 + d] =
			    (int) (((float) mode7_tempi2[offs1 + x2] * (1.0 -
				    x)) + ((float) mode7_tempi2[offs1 + x2 +
				    1] * x));
			mode7_charsi[offs2 + d] =
			    (int) (((float) mode7_tempi[offs1 + x2] * (1.0 -
				    x)) + ((float) mode7_tempi[offs1 + x2 +
				    1] * x));
			x += (11.0 / 15.0);
			if (x >= 1.0) {
				x2++;
...
static inline void mode7_render(uint8_t dat)
{
	...
	for (c = 0; c < 16; c++) {
		if (mode7_flashx && !mode7_flashon)
			b->line[scry][scrx + c + 16] = off;
		else if (mode7_dblx)
			b->line[scry][scrx + c + 16] =
			    on[mode7_px[sc & 1][t] & 15];
		else
			b->line[scry][scrx + c + 16] =
			    on[mode7_px[vid_interlace & interlline][t] & 15];
		t++;
	}
	...
	if (vid_linedbl) {
		...
		for (c = 0; c < 16; c++) {
			if (mode7_flashx && !mode7_flashon)
				b->line[scry + 1][scrx + c + 16] = off;
			else if (mode7_dblx)
				b->line[scry + 1][scrx + c + 16] =
				    on[mode7_px[sc & 1][t] & 15];
			else
				b->line[scry + 1][scrx + c + 16] =
				    on[mode7_px[1][t] & 15];
...
void video_init()
{
	...
	col0 =
	    collook[0] | (collook[0] << 8) | (collook[0] << 16) | (collook[0]
	    << 24);
	...
	for (c = 0; c < 16; c++) {
		for (d = 0; d < 64; d++) {
			mode7_lookup[d & 7][(d >> 3) & 7][c] =
			    makecol((((d & 1) * c) * 255) / 15 +
			    ((((d & 8) >> 3) * (15 - c)) * 255) / 15,
			    ((((d & 2) >> 1) * c) * 255) / 15 +
			    ((((d & 16) >> 4) * (15 - c)) * 255) / 15,
			    ((((d & 4) >> 2) * c) * 255) / 15 +
...
//        set_palette(beebpal);
	...
	for (c = 0; c < 256; c++)
		inverttbl[c] =
		    makecol((63 - pal[c].r) << 2, (63 - pal[c].g) << 2,
		    (63 - pal[c].b) << 2);
}
...
//                        scrx=0;
		...
		if (dispen) {
			...
			if (scrx < 1280) {
				if ((crtc[8] & 0x30) == 0x30 || ((sc & 8)
					&& !(ula_ctrl & 2))) {
					for (c = 0;
					    c < ((ula_ctrl & 0x10) ? 8 : 16);
					    c++) {
						((uint32_t *)
						    b->line[scry])[(scrx +
							c) >> 2] = col0;
			...
			if (charsleft) {
				...
				for (c = 0; c < ((ula_ctrl & 0x10) ? 8 : 16);
				    c += 4) {
					*((uint32_t *) & b->line[scry][scrx +
						c]) = col0;
				}
