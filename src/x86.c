...
//                        cs=CS<<4;
                        ...
                        lastcs=CS;
                        temp=readmembl(cs+pc); pc++;
                        if (temp==0xE0 && CL==0x32) bem_log(LOG_DEBUG, "XIOS call %02X %04X:%04X\n",readmembl(ds+DX),CS,pc);
/*                        if (temp==0x45)
                        {
                                printf("OSFILE %02X\n",AL);
                        }
...
//                        }
                ...
                pc&=0xFFFF;
//                if (CS==0x1490 && pc==0x3BBA) bem_log(LOG_DEBUG, "Here from %04X:%04X %08X %02X\n",oldcs,oldpc,old8,opcode);

//                if (CS==0x6012 && pc==3) bem_log(LOG_DEBUG, "XIOS direct call %02X %04X %04X %04X %04X\n",AL,CX,DX,BX,SI);
