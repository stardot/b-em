...
static void fetcheal()
{
	if (!mod && rm == 6) {
		...
		case 0:
			...
			break;
		case 1:
			eaaddr = (uint16_t) (signed char) readmembl(cs + pc);
			pc++;
			break;
		case 2:
			eaaddr = getword();
...
static uint16_t readmemwlx86(uint32_t seg, uint32_t addr)
{
	...
	if (addr2 < 0xE0000)
		return *(uint16_t *) (&x86ram[addr2]);
//        if (addr2<0xC0000) return *(uint16_t *)(&x86ram[addr2-0x40000]);
	if (addr2 > 0xF0000)
		return *(uint16_t *) (&x86rom[addr2 & 0x3FFF]);
	return 0xFFFF;
}
...
static void x86dumpregs()
{
	...
	for (c = 0xE0000; c < 0x100000; c += 0x4000)
		fwrite(x86rom, 16 * 1024, 1, f);
	fclose(f);
	printf
	    ("AX=%04X BX=%04X CX=%04X DX=%04X DI=%04X SI=%04X BP=%04X SP=%04X\n",
	    AX, BX, CX, DX, DI, SI, BP, SP);
	printf("PC=%04X CS=%04X DS=%04X ES=%04X SS=%04X FLAGS=%04X\n", pc, CS,
	    DS, ES, SS, flags);
	printf("%04X:%04X %08X %08X %08X\n", oldcs, oldpc, old8, old82,
	    old83);
	printf("%i %04X %04X\n", x86ins, pc, x86pc);
/*        if (is386)
...
void x86_init()
{
	...
	char            fn[512];
	if (!x86ram)
		x86ram = malloc(0x100000);
	if (!x86rom)
		x86rom = malloc(0x4000);
	x86makeznptable();
...
//        printf("OUT %04X %02X %04X:%04X\n",port,val,cs>>4,pc);
	...
	case 0xFF28:
		...
		return;
	case 0xFFC0:
		x86sa = (x86sa & 0xFF00) | val;
		x86src = x86sa + ((x86ss & 0xF) << 16);
		return;
	case 0xFFC1:
		x86sa = (x86sa & 0xFF) | (val << 8);
		x86src = x86sa + ((x86ss & 0xF) << 16);
		return;		//printf("SRC now %05X %04X:%04X\n",x86src,CS,pc); return;
	case 0xFFC2:
		x86ss = (x86ss & 0xFF00) | val;
		x86src = x86sa + ((x86ss & 0xF) << 16);
		return;
	case 0xFFC3:
		x86ss = (x86ss & 0xFF) | (val << 8);
		x86src = x86sa + ((x86ss & 0xF) << 16);
		return;		//printf("SRC now %05X %04X:%04X\n",x86src,CS,pc); return;
	case 0xFFC4:
		x86da = (x86da & 0xFF00) | val;
		x86dst = x86da + ((x86ds & 0xF) << 16);
		return;
	case 0xFFC5:
		x86da = (x86da & 0xFF) | (val << 8);
		x86dst = x86da + ((x86ds & 0xF) << 16);
		return;		//printf("DST now %05X %04X:%04X\n",x86dst,CS,pc); return;
	case 0xFFC6:
		x86ds = (x86ds & 0xFF00) | val;
		x86dst = x86da + ((x86ds & 0xF) << 16);
		return;
	case 0xFFC7:
		x86ds = (x86ds & 0xFF) | (val << 8);
		x86dst = x86da + ((x86ds & 0xF) << 16);
		return;		//printf("DST now %05X %04X:%04X\n",x86dst,CS,pc); return;
	case 0xFFCA:
		x86ena = (x86ena & 0xFF00) | val;
		return;
	case 0xFFCB:
		x86ena = (x86ena & 0xFF) | (val << 8);
		return;
	}
	if ((port & ~0xF) == 0x80) {
 ...
 opcodestart:
		...
		if (x86output && /*cs<0xF0000 && */ !ssegs)	//opcode!=0x26 && opcode!=0x36 && opcode!=0x2E && opcode!=0x3E)
		{
			if ((opcode != 0xF2 && opcode != 0xF3)
				...
				if (!skipnextprint)
					printf
					    ("%04X:%04X : %04X %04X %04X %04X %04X %04X %04X %04X %04X %04X %04X %04X %02X %04X\n",
					    cs, pc, AX, BX, CX, DX, CS, DS,
					    ES, SS, DI, SI, BP, SP, opcode,
					    flags, rmdat);
				skipnextprint = 0;
//                                ins++;
/*                                if (ins==50000)
...
//                        break;
		...
		case 0x60:
			 /*PUSHA*/ writememwl(ss, ((SP - 2) & 0xFFFF), AX);
			...
			break;
		case 0x61:
			 /*POPA*/ DI = readmemwl(ss, ((SP) & 0xFFFF));
...
//#if 0
		...
		case 0x69:	/*IMUL r16 */
			...
			tempw2 = getword();
			templ =
			    ((int) (signed short) tempw) *
			    ((int) (signed short) tempw2);
//                        printf("%04X*%04X = %08X\n",tempw,tempw2,templ);
			if ((templ >> 16) != 0 && (templ >> 16) != 0xFFFF)
				flags |= C_FLAG | V_FLAG;
			else
				flags &= ~(C_FLAG | V_FLAG);
...
//                        #endif
		...
		case 0x70:
			 /*JO*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			if (flags & V_FLAG) {
				pc += offset;
				tubecycles -= 9;
			...
			break;
		case 0x71:
			 /*JNO*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			if (!(flags & V_FLAG)) {
				pc += offset;
				tubecycles -= 9;
			...
			break;
		case 0x72:
			 /*JB*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			if (flags & C_FLAG) {
				pc += offset;
				tubecycles -= 9;
			...
			break;
		case 0x73:
			 /*JNB*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			if (!(flags & C_FLAG)) {
				pc += offset;
				tubecycles -= 9;
			...
			break;
		case 0x74:
			 /*JZ*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			if (flags & Z_FLAG) {
				pc += offset;
				tubecycles -= 9;
			...
			break;
		case 0x75:
			 /*JNZ*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			if (!(flags & Z_FLAG)) {
				pc += offset;
				tubecycles -= 9;
			...
			break;
		case 0x76:
			 /*JBE*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			...
			break;
		case 0x77:
			 /*JNBE*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			if (!(flags & (C_FLAG | Z_FLAG))) {
				pc += offset;
				tubecycles -= 9;
			...
			break;
		case 0x78:
			 /*JS*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			if (flags & N_FLAG) {
				pc += offset;
				tubecycles -= 9;
			...
			break;
		case 0x79:
			 /*JNS*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			if (!(flags & N_FLAG)) {
				pc += offset;
				tubecycles -= 9;
			...
			break;
		case 0x7A:
			 /*JP*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			if (flags & P_FLAG) {
				pc += offset;
				tubecycles -= 9;
			...
			break;
		case 0x7B:
			 /*JNP*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			if (!(flags & P_FLAG)) {
				pc += offset;
				tubecycles -= 9;
			...
			break;
		case 0x7C:
			 /*JL*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			...
			break;
		case 0x7D:
			 /*JNL*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			temp = (flags & N_FLAG) ? 1 : 0;
			temp2 = (flags & V_FLAG) ? 1 : 0;
			if (temp == temp2) {
			...
			break;
		case 0x7E:
			 /*JLE*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			temp = (flags & N_FLAG) ? 1 : 0;
			temp2 = (flags & V_FLAG) ? 1 : 0;
			...
			break;
		case 0x7F:
			 /*JNLE*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			temp = (flags & N_FLAG) ? 1 : 0;
			temp2 = (flags & V_FLAG) ? 1 : 0;
			if (!((flags & Z_FLAG) || (temp != temp2))) {
...
//                              printf("SS loaded with %04X %04X:%04X %04X %04X %04X\n",ss>>4,cs>>4,pc,CX,DX,es>>4);
		...
		case 0x98:
			 /*CBW*/ AH = (AL & 0x80) ? 0xFF : 0;
			tubecycles -= 2;
			break;
		case 0x99:
			 /*CWD*/ DX = (AX & 0x8000) ? 0xFFFF : 0;
			tubecycles -= 4;
			break;
		case 0x9A:	/*CALL FAR */
			tempw = getword();
...
//                        if (x86output) printf("RET to %04X %05X\n",pc,ss+SP);
		...
		case 0xC7:	/*MOV w,#16 */
			...
			break;
		case 0xC8:
			 /*ENTER*/ tempw3 = getword();
			tempi = readmembl(cs + pc);
			pc++;
			writememwl(ss, ((SP - 2) & 0xFFFF), BP);
			SP -= 2;
			tempw2 = SP;
			if (tempi > 0) {
				while (--tempi) {
				...
				writememwl(ss, ((SP - 2) & 0xFFFF), tempw2);
				SP -= 2;
			}
			BP = tempw2;
			SP -= tempw3;
...
//                        cs=CS<<4;
		...
		case 0xCD:
			...
			temp = readmembl(cs + pc);
			pc++;
			if (temp == 0xE0 && CL == 0x32)
				bem_debugf("XIOS call %02X %04X:%04X\n",
...
//                                exit(-1);
		...
		case 0xE0:
			 /*LOOPNE*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			CX--;
			if (CX && !(flags & Z_FLAG)) {
				pc += offset;
			...
			break;
		case 0xE1:
			 /*LOOPE*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			CX--;
			if (CX && (flags & Z_FLAG)) {
				pc += offset;
			...
			break;
		case 0xE2:
			 /*LOOP*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			CX--;
			if (CX) {
				pc += offset;
			...
			break;
		case 0xE3:
			 /*JCXZ*/ offset = (signed char) readmembl(cs + pc);
			pc++;
			if (!CX) {
				pc += offset;
				tubecycles -= 11;
...
//                        cs=CS<<4;
			...
			break;
		case 0xEB:	/*JMP rel */
			offset = (signed char) readmembl(cs + pc);
			pc++;
			pc += offset;
			tubecycles -= 13;
			break;
...
//                        }
			...
			break;
			printf
			    ("Bad opcode %02X at %04X:%04X from %04X:%04X %08X\n",
			    opcode, cs >> 4, pc, old8 >> 16, old8 & 0xFFFF,
			    old82);
			x86dumpregs();
			exit(-1);
		}
