...
static int16_t  snwaves[5][32] = {
	    ...
	    -127, -127, -127, -127, -127, -127, -127},
	{
	    ...
	    120, 127},
	{
		    16, 32, 48, 64, 80, 96, 112, 128, 112, 96, 80, 64, 48, 32,
		    16, 0,
		    -16, -32, -48, -64, -80, -96, -112, -128, -112, -96, -80,
	    -64, -48, -32, -16, 0},
	{
		    0, 24, 48, 70, 89, 105, 117, 124, 126, 124, 117, 105, 89,
		    70, 48, 24,
		    0, -24, -48, -70, -89, -105, -117, -124, -126, -124, -117,
	    -105, -90, -70, -48, -25}
};
...
void sn_fillbuf(int16_t * buffer, int len)
{
	...
	for (d = 0; d < len; d++) {
		for (c = 0; c < 3; c++) {
			...
			if (sn_latch[c] > 256)
				buffer[d] +=
				    (int16_t) (snwaves[curwave][sn_stat[c]] *
				    volslog[sn_vol[c]]);
			else
				buffer[d] +=
				    (int16_t) (volslog[sn_vol[c]] * 127);
		...
		if (!(sn_noise & 4)) {
			...
			buffer[d] +=
			    (((sn_shift & 1) ^ 1) * 127 * volslog[sn_vol[0]] *
			    2);
...
void sn_write(uint8_t data)
{
	...
	if (data & 0x80) {
		...
		case 0x60:
			sn_shift = 0x4000;
			if ((data & 3) != (sn_noise & 3))
				sn_count[0] = 0;
			sn_noise = data & 0xF;
			if ((data & 3) == 3)
				sn_latch[0] = sn_latch[1];
			else
				sn_latch[0] = 0x400 << (data & 3);
			break;
		case 0x70:
			data &= 0xF;
			sn_vol[0] = 0xF - data;
