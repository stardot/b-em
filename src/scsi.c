...
static void Execute(void)
{
	...
	if (scsi.cmd[0] <= 0x1f) {
		bem_debugf
		    ("scsi lun %d: Execute 0x%02x, Param 1=0x%02x, Param 2=0x%02x, Param 3=0x%02x, Param 4=0x%02x, Param 5=0x%02x, Phase = %d, PC = 0x%04x\n",
		    scsi.lun, scsi.cmd[0], scsi.cmd[1], scsi.cmd[2],
		    scsi.cmd[3], scsi.cmd[4], scsi.cmd[5], scsi.phase, pc);
	} else {
		bem_debugf
		    ("scsi lun %d: Execute 0x%02x, Param 1=0x%02x, Param 2=0x%02x, Param 3=0x%02x, Param 4=0x%02x, Param 5=0x%02x, Param 6=0x%02x, Param 7=0x%02x, Param 8=0x%02x, Param 9=0x%02x, Phase = %d, PC = 0x%04x\n",
		    scsi.lun, scsi.cmd[0], scsi.cmd[1], scsi.cmd[2],
		    scsi.cmd[3], scsi.cmd[4], scsi.cmd[5], scsi.cmd[6],
		    scsi.cmd[7], scsi.cmd[8], scsi.cmd[9], scsi.phase, pc);
	}
	...
	if ((dat = fopen(name, "rb+"))) {
		...
		if ((dsc = fopen(name, "rb+"))) {
			if (fread(geom, sizeof geom, 1, dsc) == 1) {
				...
				bem_errorf("scsi lun %d: corrupt dsc file %s",
				    lun, name);
			fclose(dsc);
		} else
			bem_warnf
			    ("scsi lun %d: unable to open dsc file %s: %s",
			    lun, name, strerror(errno));
		if (SCSISize[lun] == 0) {
			...
			if ((dsc = fopen(name, "wb"))) {
				...
				if (fwrite(geom, sizeof geom, 1, dsc) != 1)
					bem_warnf
					    ("scsi lun %d: unable to write to dsc file %s: %s",
					    lun, name, strerror(errno));
				fclose(dsc);
			}
		}
		...
		bem_warnf("scsi lun %d: unable to open data file %s: %s", lun,
		    name, strerror(errno));
}
