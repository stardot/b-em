...
//        bem_log(LOG_DEBUG, "DoSave state\n");
...
void savestate_doload()
{
        ...
        {
                fclose(f);
                bem_log(LOG_ERROR, "Not a B-em v2.x save state.");
                return;
        }
        
        curmodel = getc(f);
        selecttube = curtube = -1;
        bem_log(LOG_DEBUG, "Restart BBC");
        main_restart();
        bem_log(LOG_DEBUG, "Done!");
        ...
        bem_log(LOG_DEBUG, "Loadstate done!");
