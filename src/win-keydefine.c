...
HWND            khwnd = NULL;
LRESULT CALLBACK KeyWindowProcedure(HWND hwnd, UINT message, WPARAM wParam,
    LPARAM lParam);
...
void getkey(HWND parent)
{
	...
	khwnd = CreateWindowEx(0,	/* Extended possibilites for variation */
	    szClasskey,		/* Classname */
	    "Press key to define",	/* Title Text */
	    WS_OVERLAPPEDWINDOW & ~WS_SIZEBOX,	/* default window */
	    CW_USEDEFAULT,	/* Windows decides the position */
	    CW_USEDEFAULT,	/* where the window ends up on the screen */
	    240, 80, parent,	/* The window is a child-window to desktop */
	    NULL, hinstance,	/* Program Instance handler */
    ...
    LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		hwndCtrl =
		    CreateWindow("STATIC", s,
		    WS_CHILD | SS_SIMPLE | WS_VISIBLE, 4, 4, 240 - 10, 16,
		    hwnd, NULL, hinstance, NULL);
		hwndCtrl =
		    CreateWindow("STATIC", "Assigned to PC key(s): ",
		    WS_CHILD | SS_SIMPLE | WS_VISIBLE, 4, 20, 240 - 10, 16,
		    hwnd, NULL, hinstance, NULL);
		s[0] = 0;
		for (c = 0; c < 128; c++) {
		...
		hwndCtrl =
		    CreateWindow("STATIC", s,
		    WS_CHILD | SS_SIMPLE | WS_VISIBLE, 4, 36, 240 - 10, 16,
		    hwnd, NULL, hinstance, NULL);
		break;
...
BOOL CALLBACK redefinedlgproc(HWND hdlg, UINT message, WPARAM wParam,
    LPARAM lParam)
{
	switch (message) {
	case WM_INITDIALOG:
	...
	case WM_COMMAND:
		...
		if (LOWORD(wParam) >= Button1
		    && LOWORD(wParam) <= (Button1 + 128) && !khwnd) {
...
{
	if (MASTER)
		DialogBox(hinstance, TEXT("RedefineM"), ghwnd,
		    redefinedlgproc);
	else
		DialogBox(hinstance, TEXT("Redefine"), ghwnd,
		    redefinedlgproc);
}
#endif
