...
void cmos_update(uint8_t IC32, uint8_t sdbval)
{
        ...
        cmos_old = IC32 & 4;
//        bem_log(LOG_DEBUG, "CMOS update %i %i %i\n",cmos_rw,cmos_strobe,cmos_old);
...
void cmos_writeaddr(uint8_t val)
{
        if (val&0x80) /*Latch address*/
           cmos_addr = sdbval & 63;
        cmos_ena = val & 0x40;
//        bem_log(LOG_DEBUG, "CMOS writeaddr %02X %02X %02X\n",val,sdbval,cmos_addr);
}

uint8_t cmos_read()
{
//        bem_log(LOG_DEBUG, "CMOS read ORAnh %02X %02X %i %02X %i\n",cmos_addr,cmos[cmos_addr],cmos_ena,IC32,cmos_rw);
...
void cmos_load(MODEL m)
{
        ...
        else
        {
                ...
                bem_log(LOG_DEBUG, "CMOS Opening %s", fn);
                f=x_fopen(fn, "rb");
                if (!f)
                {
                        bem_log(LOG_ERROR, "unable to load CMOS file %s: %s",
			    fn, strerror(errno));
                        memset(cmos, 0, 64);
                        return;
                }
...
void cmos_save(MODEL m)
{
        ...
        {
                sprintf(fn, "%s%s", exedir, m.cmos);
                bem_log(LOG_DEBUG, "CMOS Opening");
                f=x_fopen(fn, "wb");
                fwrite(cmos, 64, 1, f);
                fclose(f);
        }
