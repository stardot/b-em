...
void al_init()
{
	...
	for (c = 0; c < 4; c++)
		alBufferData(buffers[c], AL_FORMAT_STEREO16, tempbuf, BUFLEN,
		    31250);
...
//        printf("InitAL\n");
	...
	for (c = 0; c < 4; c++)
		alBufferData(buffersdd[c], AL_FORMAT_STEREO16, tempbufdd,
		    4410 * 4, 44100);
...
//bem_debug("BufferData\n");
		alBufferData(buffer, AL_FORMAT_STEREO16, zbuf, 4410 * 4,
		    44100);
		check();
