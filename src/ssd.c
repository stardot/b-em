...
//        printf("Read sector %i %i %i %i\n",drive,side,track,sector);
	...
	if (!ssd_f[drive] || (side && !dsd[drive]) || density
	    || track != ssd_trackc[drive]) {
		ssd_notfound = 500;
///                printf("Not found!\n");
		return;
	}
...
//                printf("Read %i\n",ssdreadpos);
		fdc_data(trackinfo[ssd_drive][ssd_side][(ssd_sector << 8) +
			ssd_readpos]);
		ssd_readpos++;
		if (ssd_readpos == 256) {
			ssd_inread = 0;
...
//                        exit(-1);
		...
		trackinfo[ssd_drive][ssd_side][(ssd_sector << 8) +
		    ssd_readpos] = c;
		ssd_readpos++;
		if (ssd_readpos == 256) {
			ssd_inwrite = 0;
