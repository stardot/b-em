...
//        printf("path now %s\n",path);
	...
	for (c = 0; c < 32; c++) {
		sinewave[c] =
		    (int) (sin((float) c * ((2.0 * PI) / 32.0)) * 128.0);
	}
}
	...
	for (c = 0; c < 4410; c++) {
		if (tnoise_sstat >= 0) {
			if (tnoise_spos >= tsamples[tnoise_sstat]->len) {
				...
				tapebuffer[c] += ((int16_t) ((((int16_t *)
						tsamples[tnoise_sstat]->data)[
						(int) tnoise_spos]) ^ 0x8000)
				    / 4);
				tnoise_spos +=
				    ((float) tsamples[tnoise_sstat]->freq /
				    44100.0);
			}
		}
	}
