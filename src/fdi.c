...
{
	if (fdi_h[drive])
		fdi2raw_header_free(fdi_h[drive]);
	if (fdi_f[drive])
		fclose(fdi_f[drive]);
	fdi_f[drive] = NULL;
}
...
//        printf("Track start %i\n",track);
	if (track < 0)
		track = 0;
	if (track > fdi_lasttrack[drive])
		track = fdi_lasttrack[drive] - 1;
	c = fdi2raw_loadtrack(fdi_h[drive],
	    (uint16_t *) fdi_trackinfo[drive][0][0], (uint16_t *) fdi_timing,
	    track << fdi_sides[drive], &fdi_tracklen[drive][0][0],
	    &fdi_trackindex[drive][0][0], NULL, 0);
	if (!c)
		memset(fdi_trackinfo[drive][0][0], 0,
		    fdi_tracklen[drive][0][0]);
	c = fdi2raw_loadtrack(fdi_h[drive],
	    (uint16_t *) fdi_trackinfo[drive][0][1], (uint16_t *) fdi_timing,
	    track << fdi_sides[drive], &fdi_tracklen[drive][0][1],
	    &fdi_trackindex[drive][0][1], NULL, 1);
	if (!c)
		memset(fdi_trackinfo[drive][0][1], 0,
		    fdi_tracklen[drive][0][1]);
	if (fdi_sides[drive]) {
		c = fdi2raw_loadtrack(fdi_h[drive],
		    (uint16_t *) fdi_trackinfo[drive][1][0],
		    (uint16_t *) fdi_timing, (track << fdi_sides[drive]) + 1,
		    &fdi_tracklen[drive][1][0], &fdi_trackindex[drive][1][0],
		    NULL, 0);
		if (!c)
			memset(fdi_trackinfo[drive][1][0], 0,
			    fdi_tracklen[drive][1][0]);
		c = fdi2raw_loadtrack(fdi_h[drive],
		    (uint16_t *) fdi_trackinfo[drive][1][1],
		    (uint16_t *) fdi_timing, (track << fdi_sides[drive]) + 1,
		    &fdi_tracklen[drive][1][1], &fdi_trackindex[drive][1][1],
		    NULL, 1);
		if (!c)
			memset(fdi_trackinfo[drive][1][1], 0,
			    fdi_tracklen[drive][1][1]);
	} else {
		...
		fdi_trackindex[drive][1][0] = fdi_trackindex[drive][1][1] =
		    100;
	}
...
static int      readidpoll = 0, readdatapoll = 0, fdi_nextsector =
    0, inreadop = 0;
...
//                printf("Looping! %i\n",fdipos);
	...
	tempi =
	    fdi_trackinfo[fdi_drive][fdi_side][fdi_density][((fdi_pos >> 3) &
		0xFFFF) ^ 1] & (1 << (7 - (fdi_pos & 7)));
	fdi_pos++;
	fdi_buffer <<= 1;
		...
		if (!pollbitsleft) {
			...
			if (readidpoll) {
				fdi_sectordat[5 - pollbytesleft] =
				    decodefm(fdi_buffer);
				if (fdi_inreadaddr && pollbytesleft > 1)
					fdc_data(fdi_sectordat[5 -
						pollbytesleft]);
				if (!pollbytesleft) {
